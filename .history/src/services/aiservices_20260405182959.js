...
const isExpectedProviderError = (error) => {
  ...
  return status === 404 || status === 429 || code === "insufficient_quota";
};

const PROVIDER_COOLDOWN_MS = 10 * 60 * 1000;

const isProviderCooldownActive = () => Date.now() < providerCooldownUntil;

const activateProviderCooldown = () => {
  providerCooldownUntil = Date.now() + PROVIDER_COOLDOWN_MS;
};

const logAiFallbackWarning = (error) => {
  const now = Date.now();

  // Throttle repeated warnings so terminal stays readable.
  if (now - lastAiWarningAt < 60_000) {
    return;
  }
  ...
  const message = error?.message || "AI provider unavailable";

  console.warn(`AI fallback active (status: ${status}, code: ${code}): ${message}`);
};
...
export const analyzeCode = async (code, language) => {
  try {
    if (!genAI || isProviderCooldownActive()) {
...
${code}`;
    ...
    if (isExpectedProviderError(error)) {
      ...
      // Keep terminal quiet for expected provider failures.
      if (process.env.AI_DEBUG_LOGS === "true") {
        logAiFallbackWarning(error);
      }
    } else {
